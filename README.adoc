...
Simple client and server applications that simulate a shopping cart for kicking the tires on the-state-of-the-art Java microservice frameworks. The server is built using link:https://micronaut.io/[Micronaut]. The client uses the link:https://openjdk.java.net/groups/net/httpclient/[HTTP Client] library introduced in Java 11.
